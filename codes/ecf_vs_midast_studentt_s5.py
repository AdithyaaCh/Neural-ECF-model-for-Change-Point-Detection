...
DIST = "student_t"
N = 1000
WINDOW = 150              # ECF window length L
GAP = 10
SCAN_STEP = 10         
SMOOTH = 5
M_FREQ = 256
...
MIDAST_SHIFT = int(os.getenv("MIDAST_SHIFT", 5))   # s = 5 for MIDAST

OUTDIR = os.path.join(ROOT, f"results_s5_studentt_d{DIM}")
...
def make_studentt(n, n_star, rho1, rho2, nu1, nu2, d=DIM):
...
# MIDAST runner  --  s = MIDAST_SHIFT, k = w/(100*s) rule of thumb (Appendix A)
def _select_w_k(test_name):
    """w via Algorithm 1 (cached per test/dist/dim). k via the paper rule of
    thumb k = w/(100*s) with s = MIDAST_SHIFT. Algorithm 2 is NOT used."""
...
def run_midast(X, test_name):
    """MIDAST via the vendored ChangeDetector with shift s=MIDAST_SHIFT and
    rule-of-thumb k. Single change point -> max_no_changes=1, based_on='statistic'."""
    w, k = _select_w_k("KSTest" if test_name == "KSTest" else "MMDTest")
    s = MIDAST_SHIFT
...
def _cell_worker_A(args):
    ...
    for trial in range(n_trials):
        ...
        X, cp = make_studentt(N, N // 2, RHO1, r2, NU1, nu2)
        res = evaluate_all(X, cp, ecf)
        for m, (e, t) in res.items():
            rows.append({"Method": m, "rho2": r2, "nu2": nu2, "MAE": e, "time_s": t})
    return rows
...
def experiment_A(ecf):
    total_cells = len(NU2_GRID) * len(RHO2_GRID)
    total_series = total_cells * N_TRIALS_A
    n_workers = min(N_WORKERS, total_cells)
    partial = os.path.join(OUTDIR, "raw_A_partial.csv")

    cells = [
        (nu2, r2, N_TRIALS_A, idx * N_TRIALS_A)
        for idx, (nu2, r2) in enumerate(
            (nu, r) for nu in NU2_GRID for r in RHO2_GRID)
    ]
    ...
    print(f"\n=== Experiment A [s={MIDAST_SHIFT}] : {len(NU2_GRID)}x{len(RHO2_GRID)} grid, "
    ...
    with ProcessPoolExecutor(max_workers=n_workers) as pool:
        ...
        for fut in as_completed(futures):
            ...
            completed += 1
            nu2, r2 = futures[fut][0], futures[fut][1]
            print(f"  nu2={nu2:<5} rho2={r2:<5}  [{completed}/{total_cells} cells done]",
                  flush=True)
...
def timing_benchmark(ecf, n=N_TIMING):
    ...
    rows = []
    for k in range(n):
        X, _ = make_studentt(N, N // 2, RHO1, -0.5, NU1, 2.0)
    ...
    ax.set_title(f"Runtime per series, Student-t d={DIM}  (MIDAST s={MIDAST_SHIFT})",
                 fontweight="bold")
...
def _heatmaps(df, value, cmap, fname, title, fmt=".0f"):
    ...
    for ax, m in zip(axes[0], METHODS):
        piv = (df[df.Method == m].groupby(["nu2", "rho2"])[value]
...
def plot_A(df):
    _heatmaps(df, "MAE", "rocket_r", "expA_mae_heatmaps.png",
              f"Exp A  -  Localisation Error (MAE), Student-t d={DIM}  (MIDAST s={MIDAST_SHIFT})",
              fmt=".0f")
...
def main():
    np.random.seed(SEED)
    print(f"Mode: {'SMOKE' if SMOKE else 'FULL'} | dist={DIST} | d={DIM} | "
          f"MIDAST shift s={MIDAST_SHIFT} | ECF stride={SCAN_STEP} | "
